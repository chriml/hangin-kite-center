...
import { auditHtml, auditCss, auditExport, readSiteContract, digest } from '../scripts/privacy-audit.mjs';
...
test('allows only the reviewed non-submitting calculator form on its own route', () => {
  const html = '<form aria-labelledby="trip-heading"><fieldset disabled><input id="rider-weight"><button type="submit">Calculate</button></fieldset></form>';
  const url = `${origin}/kite-size-guide/`;
  assert.deepEqual(auditHtml({html, url}).findings, []);
  assert.ok(auditHtml({html, url:origin}).findings.some(f => f.code === 'unsupported-active-content'));
  for (const modified of [html.replace('<form ', '<form action="/collect" '), html.replace('<input ', '<input name="weight" '), html.replace('<button ', '<button formaction="/collect" ')]) {
    assert.ok(auditHtml({html:modified, url}).findings.some(f => f.code === 'unsupported-active-content'));
  }
  const externalControls = html.replace('<form ', '<form id="calc" ') + '<input form="calc" name="health" value="private"><button form="calc" formaction="/collect">Send</button>';
  assert.ok(auditHtml({html:externalControls, url}).findings.some(f => f.code === 'unsupported-active-content'));
});
...
test('a reviewed client boundary passes only while its exact source matches', async t => {
  const source = '"use client"; export default function Calculator(){ return null; }';
  const root = await fixture(t, {...baseline,
    'components/calculator.tsx':source,
    'scripts/reviewed-client-boundaries.json':JSON.stringify({'components/calculator.tsx':{sha256:digest(source), review:'docs/review.md'}}),
  });
  assert.deepEqual((await auditExport({root, origin, expectedRoutes:['/']})).findings, []);
  await writeFile(path.join(root, 'components/calculator.tsx'), source + '\nfetch("/collect");');
  assert.ok((await auditExport({root, origin, expectedRoutes:['/']})).findings.some(f => f.code === 'authored-client-runtime'));
});
test('literal route spreads preserve full export coverage and explicit sitemap exclusions', async t => {
  const root = await fixture(t, {...baseline, 'out/events/index.html':'<h1>Events</h1>',
    'content/site.ts':`export const siteConfig = {origin: '${origin}'} as const;
      const pending = ['/events/'] as const;
      export const publicRoutes = ['/', ...pending] as const;
      export const noindexRoutes = [...pending] as const;`,
  });
  const contract = await readSiteContract(root);
  assert.deepEqual(contract.expectedRoutes, ['/', '/events/']);
  assert.deepEqual(contract.expectedIndexableRoutes, ['/']);
  assert.deepEqual((await auditExport({root})).findings, []);
  await rm(path.join(root, 'out/events/index.html'));
  assert.ok((await auditExport({root})).findings.some(f => f.code === 'route-coverage'));
});
test('route reader rejects executable route expressions without executing them', async t => {
  const root = await fixture(t, {'content/site.ts':`export const siteConfig = {origin: '${origin}'}; export const publicRoutes = ['/', ...fetch('/route-list')];`});
  await assert.rejects(readSiteContract(root));
});
