...
The site has explicit crawlable routes declared in `publicRoutes` (the nine original marketing pages plus inquiry and accessibility information pages), unique metadata, self-canonicals, local social images, sitemap and robots output, breadcrumbs on supporting pages, semantic headings, and visible content-backed JSON-LD. The canonical origin is `https://www.hanginkitecenter.com`.
