...
export function auditHtml({html, url, frameworkAssets = new Map(), onResource}) {
  ...
  const visit = node => {
    ...
    if (['iframe', 'object', 'embed', 'form'].includes(name)) add(report, 'unsupported-active-content', `${name} requires an accepted integration`, {url});
...
export async function readSiteContract(root) {
  ...
  for (const statement of source.statements) if (ts.isVariableStatement(statement)) for (const declaration of statement.declarationList.declarations) {
    ...
    if (name === 'publicRoutes' && initializer && ts.isArrayLiteralExpression(initializer)) {
      contract.expectedRoutes = initializer.elements.map(element => { if (!ts.isStringLiteral(element)) throw new Error('Routes must be literal'); return element.text; });
    }
...
export async function auditExport({root = process.cwd(), origin, expectedRoutes} = {}) {
  const report = {...result(), frameworkAssets: new Map(), routes: [], limitations};
  if (!origin || !expectedRoutes) ({origin, expectedRoutes} = await readSiteContract(root));
  ...
  for (const directory of ['app', 'components', 'content', 'lib']) for (const file of await files(path.join(root, directory), true)) if (/\.[cm]?[jt]sx?$/.test(file)) {
    ...
    for (const statement of source.statements) {
      ...
      if (statement.expression.text === 'use client') add(report, 'authored-client-runtime', 'Client boundary requires a reviewed runtime integration', {file});
    }
  }
  ...
  try {
    ...
    if (routes.length !== expectedRoutes.length || routes.some(route => !expectedRoutes.includes(route))) throw new Error('Sitemap and typed routes differ');
