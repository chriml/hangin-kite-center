...
export async function auditDeployment({origin, expectedRoutes, canonicalOrigin = origin, frameworkAssets = new Map()}) {
  ...
  try {
    ...
    if (discovered.length !== expectedRoutes.length || discovered.some(route => !expectedRoutes.includes(route))) throw new Error('Remote sitemap differs from trusted expected routes');
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  try {
    ...
    if (local.findings.length) {
      ...
      const report = await auditDeployment({origin:process.argv[2], canonicalOrigin:contract.origin, expectedRoutes:contract.expectedRoutes, frameworkAssets:local.frameworkAssets});
