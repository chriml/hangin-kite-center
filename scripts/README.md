...
Status: Current tooling, 4 September 2026
...
Run `npm run audit:privacy:deployed -- https://preview.example` to compare a host with that local export. The local audit must pass first. The remote sitemap must contain exactly the typed public routes at the canonical origin, even when the audited host is a preview. A missing or incomplete sitemap fails; known routes are still inspected. Unexpected cross-origin redirects are reported without following them.
...
- Reject authored public/exported JavaScript files and unclassified executable script declarations. Reject local `use client` boundaries in `app/`, `components/`, `content/` and `lib/` under the current no-authored-runtime contract. The TypeScript parser reads source directives and site constants without executing application modules.
